...
public class CommandLineArgs {
	...
	private static final String OPT_DATA_ROOT_FOLDER = "root-folder";
	...
	private static final Option[] OPTIONS = {
	    ...
	    makeOption("D", OPT_SCRATCH_DIRECTORY,
	        "Specify an alternate scratch directory <dir>", "dir"),
			makeOption("R", OPT_DATA_ROOT_FOLDER,
					"Specify the root folder of the data", "dir"),
	    makeOption(null, OPT_SIZES,
	        "Report the valid sizes for the specified benchmarks", null),
	    ...
	    makeOption(null, OPT_NO_PRE_ITERATION_GC,
	        "Skip performing System.gc() before the start of each iteration",
	        null),
	    makeOption("t", OPT_THREAD_COUNT,
	        "Set the thread count to drive the workload (mutually exclusive -k)",
	        "thread_count"),
	    makeOption(
	        "k",
	        OPT_THREAD_FACTOR,
	        "Set the number of threads per CPU to drive the workload (mutually exclusive with -t)",
	        "thread_per_cpu"),
	    makeOption(
	        "f",
	        OPT_TIMEOUT_DIALATION,
	        "Set the time dialation of the timeouts in the benchmark by the given integer factor.",
	        "timeout_dialation"),
	    makeOption("v", OPT_VERBOSE, "Verbose output", null) };

	private CommandLineParser parser = new PosixParser();
	private Options options = new Options();
	private Options visibleOptions = new Options();
	...
	CommandLineArgs(String[] args) throws Exception {
		try {
			...
			if(line.hasOption(OPT_DATA_ROOT_FOLDER)) {
				String rootPath = line
						.getOptionValue(OPT_DATA_ROOT_FOLDER, "./");
				etalon.purdue.edu.base.wrapper.File.setRootPath(rootPath);
			}
	...
	static List<String> extractBenchmarkSet() throws IOException {
		List<String> benchmarks = BaseActivityHelper.singleToneIns.getAllAppNames();
	...
	public String getTimeoutDialation() {
		return line
		    .getOptionValue(OPT_TIMEOUT_DIALATION, DEFAULT_TIMEOUT_DIALATION);
	}
